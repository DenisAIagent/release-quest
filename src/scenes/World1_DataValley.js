...
export default class World1_DataValley extends Phaser.Scene {
  ...
  create() {
    this.cameras.main.setBackgroundColor('#000000'); // Noir (thème agence)
  ...
  createWorld() {
    ...
    graphics.fillStyle(0x000000, 1); // Noir (fond)
    graphics.fillRect(0, 0, worldWidth, worldHeight);

    // Ajouter une grille pour visualiser le terrain
    graphics.lineStyle(1, 0xFFFFFF, 0.1); // Blanc très transparent
    for (let x = 0; x < 50; x++) {
    ...
    for (let x = 0; x < 50; x++) {
      for (let y = 0; y < 30; y++) {
        if (x === 0 || x === 49 || y === 0 || y === 29) {
          ...
          graphics.fillStyle(0xFFFFFF, 0.2); // Blanc transparent (obstacles)
          graphics.fillRect(x * 32, y * 32, 32, 32);
        }
      }
    }
